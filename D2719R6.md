...
>
> ::: add
> [2a]{.pnum} The first parameter _P_ of an allocation or deallocation function is a _type-identity parameter_ if the type _U_ of _P_ is a specialization of `std::type_identity` ([meta.trans.other]) and, if _U_ is dependent, _U_ is the same  as ([temp.over.link]) `std::type_identity<V>` for some type _V_.
...
> - [20.2]{.pnum} [_S_ be an expression of type `std::size_t` equal to the number of bytes requested,]{.add}
> - [20.3]{.pnum} [_A_ be an expression of type `std::align_val_t` equal to the alignment requirement of the allocated type, and]{.add}
...
>
> - [20.5]{.pnum} [(_T_, _S_, _P_) if the first #initializer-clause# in _P_ (if any) has the type `std::align_val_t` after parameter transformations ([dcl.fct]), and (_T_, _S_, _A_, _P_) otherwise, or, if overload resolution fails,]{.add}
...
>
>  - [21.5]{.pnum} `new(std::align_val_t(7), f) T` results in one of the following calls:
>    ```cpp
...
>   - [9.1a.3]{.pnum} [If overload resolution selects a function _F_ using _L_ as the argument list and the deallocation functions as the set of candidates, then _F_ is the selected deallocation function. Otherwise, type-aware deallocation functions are eliminated from further consideration.]{.add}
