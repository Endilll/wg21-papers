...
>
> - [[1a.1]{.pnum} If _F_ is a type-aware allocation operator, then _F_ shall have at least three parameters, where the second parameter is termed the _size parameter_ and the third parameter is termed the _alignment parameter_.]{.add}
> - [[1a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is termed the _size parameter_.]{.add}
> - [[1a.3]{.pnum} Otherwise _F_ shall be a function that has at least one parameter, where the first parameter is termed the _size parameter_.]{.add}
>
...
::: draftnote
(CWG) Is it fine to have "size parameter" italicized three times?
:::
...
> [2a]{.pnum} [The declared return type of a deallocation operator _F_ shall be the same as ([temp.over.link]) `void`.]{.add}
> [The following restrictions apply to the parameters of _F_:]{.add}
>
> - [[2a.1]{.pnum} If _F_ is a type-aware deallocation function, then _F_ shall have at least four parameters, where]{.add}
>   - [[2a.1.1]{.pnum} the second parameter is termed the _address parameter_,]{.add}
>   - [[2a.1.2]{.pnum} the third parameter shall have a type that is the same as `std::size_t` ([support.types]), and]{.add}
>   - [[2a.1.3]{.pnum} the fourth parameter shall have a type that is the same as `std::align_val_t` ([support.dynamic]).]{.add}
> - [[2a.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is termed the _address parameter_.]{.add}
> - [[2a.3]{.pnum} Otherwise, _F_ shall be a function that has at least one parameter, where the first parameter is termed the _address parameter_.]{.add}
>
> [The address parameter of a deallocation operator that is not a destroying operator delete (see below) shall have a type that is the same as "pointer to `void`".]{.add}
> [A deallocation operator shall not have a potentially throwing exception specification ([except.spec]).]{.add}
>
> ::: add
> ::: note
...
>
> [The allocation function is selected by overload resolution using the argument list termed _K_ as specified in [over.match.new].]{.add}
> [The selected allocation function is called with arguments in _K_.]{.add}
> [If there are parameters that do not have a corresponding argument in _K_, their default arguments are used instead.]{.add}
> [The resulting list of arguments to call the selected allocation function is termed _L_.]{.add}
>
> ::: add
> ::: example
> ```cpp
...
> - [30.b]{.pnum} [_N_ be an argument list that is same as _L_, except that the value returned from the call to the selected allocation function is inserted before the first argument.]{.add}
>
...
> [The selected allocation function is called with arguments in _N_.]{.add}
>
> [31]{.pnum}
>
> ::: example
...
> ::: add
> ::: note
> [This does not affect matching deallocation functions selected in [expr.delete], because they are usual dealloaction functions, therefore they do not accept additional arguments.]{.add}
> :::
> :::
...
:::
::: draftnote
Hopefully, we'll get at least _an_ answer what "placement (de)allocation functions" are, if Core will discuss [CWG2592](https://cplusplus.github.io/CWG/issues/2592.html) "Missing definition for placement allocation/deallocation function" during the 2026-06-26 telecon.
:::
::: draftnote
(CWG) In p32, "if the implementation is allowed" sounds awkward. Is there wording somewhere else allowing or disallowing copyinh the placement arguments?
:::
...
> :::
>
> [8a]{.pnum} [The selected deallocation function is the static deallocation function as specified below, unless the following holds when selecting a deallocation function for a #delete-expression# _E_:]{.add}
>
> - [8a.1]{.pnum} [the keyword `delete` in _E_ is not preceded by the unary `​::`​ operator,]{.add}
> - [8a.2]{.pnum} [the operand of _E_ is a pointer to _cv_ `T`, where `T` is a class type or (possibly multidimensional) array thereof, and]{.add}
> - [8a.3]{.pnum} [`T` has a virtual destructor,]{.add}
>
> [in which case the selected deallocation function is the dynamic deallocation function ([class.dtor]) of the virtual destructor of the dynamic type of the operand of _E_.]{.add}
>
> ::: add
> ::: note
> [The dynamic deallocation function is never a matching deallocation function for a #new-expression#.]{.add}
> :::
> :::
>
> [8b]{.pnum} [The static deallocation function is selected for every #delete-expression#.]{.add}
>
> ::: add
> ::: note
> [The static deallocation function is selected and its semantic constraints are checked even if it does not become the selected deallocation function.]{.add}
> :::
> :::
>
...
> ::: rm
> ::: note
> [Any cv-qualifiers in the type of the deleted object are ignored when forming this argument.]{.rm}
> :::
> :::
>
> ::: add
...
> :::
>
> [10a]{.pnum} [The _deallocation address_ is:]{.add}
>
> - [10a.1]{.pnum} [the address of the deleted object, if the #delete-expression# is a single-object delete expression, or]{.add}
> - [10a.2]{.pnum} [the address of the deleted object adjusted for the array allocation overhead, if the #delete-expression# is an array delete expression, or]{.add}
> - [10a.3]{.pnum} [the value returned by the call to the allocation function, if _F_ is a matching deallocation function for a #new-expression#.]{.add}
>
...
> :::
>
> [4a]{.pnum} [If a class scope _S_ is the target scope of a type-aware allocation or deallocation operator _F_, then]{.add}
>
> - [4a.1]{.pnum} [if the name of _F_ is `operator new` or `operator delete`, _S_ shall be the target scope of an allocation operator named `operator new` and a deallocation operator named `operator delete`, or]{.add}
> - [4a.2]{.pnum} [if the name of _F_ is `operator new[]` or `operator delete[]`, _S_ shall be the target scope of an allocation operator named `operator new[]` and a deallocation operator named `operator delete[]`.]{.add}
>
> [5]{.pnum}
>
> ::: {.note .add}
> Access to the deallocation function is checked statically, even if a different one is actually executed [([expr.delete])]{.add}.
> :::
>
> ::: example
...
::: draftnote
Do we need "function" → "operator" replacement in p5? In other words, do we need to check for access to a function template prior to instantiation?
:::

## Candidate functions and argument lists [over.match.funcs.general]{.sref .unnumbered}

Do not change [over.match.funcs.general]:
...
>
> - [1.1]{.pnum} [If _E_ does not begin with `::` and `U` is a class type or array thereof, in the scope associated with `U`.]{.add}
> - [1.2]{.pnum} [Otherwise, or if no declarations were found, in the global scope.]{.add}
>
> [Each candidate that is a destroying operator delete ([basic.std.dynamic.deallocation]) is removed from the set of candidate functions.]{.add}
>
> [2]{.pnum} [Let]{.add}
>
> - [2.1]{.pnum} [_T_ be a value of type `std::type_identity<V>` ([meta.trans.other]), where `V` is a cv-unqualified version of the array element type of `U` if `U` is an array type, and a cv-unqualified version of `U` otherwise,]{.add}
> - [2.2]{.pnum} [_S_ be a value of type `std::size_t` ([support.types]) equal to the allocation size,]{.add}
> - [2.3]{.pnum} [_A_ be a value of type `std::align_val_t` ([support.dynamic]) equal to the allocation alignment, and]{.add}
> - [2.4]{.pnum} [_P_ be the #initializer-clause#s in the #expression-list# of _E_ if the #new-placement# syntax is used, and an empty set otherwise.]{.add}
>
> [Overload resolution selects an allocation function in multiple phases using different argument lists. The argument list for an allocation function consists of]{.add}
>
> - [2.5]{.pnum} [(_T_, _S_, _P_) if the first #initializer-clause# in _P_ (if any) has the type `std::align_val_t` after parameter transformations ([dcl.fct]), and (_T_, _S_, _A_, _P_) otherwise, or, if overload resolution fails,]{.add}
> - [2.6]{.pnum} [(_S_, _A_, _P_) if the type of the allocated object has new-extended alignment, and (_S_, _P_) otherwise, or, if overload resolution fails,]{.add}
> - [2.7]{.pnum} [(_S_, _P_) if the type of the allocated object has new-extended alignment, and (_S_, _A_, _P_) otherwise.]{.add}
...
>
> ::: example
> [In the following examples `T` does not have new-extended alignment and is not an array type:]{.add}
>
>  - [3.1]{.pnum} `new T` results in one of the following calls:
>    ```cpp
...
>
> ::: add
> [In the following example `T` has new-extended alignment and is not an array type:]{.add}
>
>  - [3.5]{.pnum} `new(std::align_val_t(7), f) T` results in one of the following calls:
>    ```cpp
...
> The argument list consists of the arguments used to call _F_ ([expr.new]), except that the value returned from the call to _F_ is inserted after the first argument.]{.add}
>
> ::: add
> ::: note
> [If a call to the selected allocation function uses default arguments, they can influence the selection of a matching deallocation function. The parameter-type-list of the selected matching deallocation function has to satisfy additional criteria for a program to be well-formed ([expr.new]).]{.add}
> :::
> :::
>
> [5]{.pnum}
...
> ::: add
> ::: note
> [Otherwise, the matching deallocation function is selected as specified in [expr.new].]{.add}
> :::
> :::
...
> [[over.match.delete]/1]{.pnum} [When selecting a deallocation function for a #delete-expression# _E_ whose operand is of type "pointer to _cv_ `U`", the set of candidate functions consists of the results of a search for the deallocation function's name ([expr.delete]) in the following scopes:]{.add}
>
> - [1.1]{.pnum} [If _E_ does not begin with `::` and `U` is a class type, in the scope associated with `U`.]{.add}
> - [1.2]{.pnum} [Otherwise, or if no declarations were found, in the global scope.]{.add}
>
> [If any of the candidates is a destroying operator delete, each candidate that is not a destroying operator delete is removed from the set of candidate functions.]{.add}
>
> [2]{.pnum}
>
> ::: add
> ::: note
> [When selecting a matching deallocation function for an allocation function that is not a placement allocation function ([expr.new]), the set of candidate functions is as specified in [over.match.new].]{.add}
> :::
> :::

Move the note that follows [expr.delete] paragraph 8 after paragraph 3 and apply the following changes:

> [3]{.pnum} [When selecting a deallocation function for a #delete-expression# or a matching deallocation function for an allocation function that is not a placement allocation function, each candidate that is not a usual deallocation operator ([basic.stc.dynamic.deallocation]) is removed from the set of candidate functions.]{.add}
>
> ::: note
...
> :::
>
> [4]{.pnum} [When selecting a deallocation function for a #delete-expression# or a matching deallocation function for an allocation function that is not a placement allocation function, if one of the candidates is a type-aware deallocation operator, overload resolution selects a deallocation function using an argument list that consists of, in order:]{.add}
>
> - [3.1]{.pnum} [a value of type `std::type_identity<U>` ([meta.trans.other]),]{.add}
> - [3.2]{.pnum} [a value of type "pointer to `void`" that represents the deallocation address,]{.add}
> - [3.3]{.pnum} [a value of type `std::size_t` ([support.types]) equal to the deallocation size, and]{.add}
> - [3.4]{.pnum} [a value of type `std::align_val_t` ([support.dynamic]) equal to the deallocation alignment.]{.add}
>
> [5]{.pnum}
>
> ::: add
> ::: note
> [Otherwise, or if overload resolution fails, a deallocation function is selected as specified in [expr.delete].]{.add}
> :::
> :::
