...
>
> template <typename X>
> void* S<X>::operator new(std::type_identity<S<X>>, std::size_t, std::align_val_t) { return {}; };    // a type-aware allocation function
> ```
> :::
> :::
>
...
>
> [1a]{.pnum} The [declared]{.add} return type [of an allocation function _F_]{.add} shall be [the same as ([temp.over.link])]{.add} "pointer to `void`". [The minimum number of parameters of _F_ is determined as follows:]{.add}
>
> - [[1a.1]{.pnum} If _F_ is a type-aware allocation function, then _F_ shall have at least three parameters, where the second parameter is termed the _size parameter_ and the third parameter is termed the _alignment parameter_.]{.add}
...
::: draftnote
The new wording for the return type of allocation and deallocation functions should resolve [CWG1676](https://cplusplus.github.io/CWG/issues/1676.html) "`auto` return type for allocation and deallocation functions", as it follows the approach in [CWG1669](https://cplusplus.github.io/CWG/issues/1669.html) "`auto` return type for `main`".
:::
...
> [3-]{.pnum} [The declared return type of a deallocation function _F_ shall be the same as ([temp.over.link]) `void`.]{.add}
> [The following restrictions apply to the parameters of _F_:]{.add}
>
...
>
> [The address parameter of a deallocation function that is not a destroying operator delete (see below) shall have a type that is the same as "pointer to `void`".]{.add}
> [A deallocation function shall not have a potentially throwing exception specification ([except.spec]).]{.add}
>
> ::: add
> ::: note
...
> A _usual deallocation function_ is
>
> - [3.1]{.pnum} [a type-aware deallocation function with four parameters or]{.add}
> - [3.2]{.pnum} a deallocation function [which is not a function template or function template specialization and]{.add} whose parameters after the [first]{.rm} [address parameter]{.add}
...
::: draftnote
The new wording for the return type of deallocation functions should resolve [CWG3025](https://cplusplus.github.io/CWG/issues/3025.html) "Deallocation functions returning `void`".
:::
::: draftnote
...
> [1]{.pnum} Any allocation function for a class `T` is a static member (even if not explicitly declared `static`).
>
> [2]{.pnum} [[. . .]]{.note}
>
> [3]{.pnum} Any deallocation function for a class `X` is a static member (even if not explicitly declared `static`).
>
> [[. . .]]{.example}
>
> [4]{.pnum} Since member allocation and deallocation functions are `static` they cannot be virtual.
>
> ::: note
...
> :::
>
> [4a]{.pnum} [If a class scope _S_ is the target scope of a type-aware allocation or deallocation function _F_, then]{.add}
>
> - [4a.1]{.pnum} [if the name of _F_ is `operator new` or `operator delete`, _S_ shall be the target scope of an allocation function named `operator new` and a deallocation function named `operator delete`, or]{.add}
> - [4a.2]{.pnum} [if the name of _F_ is `operator new[]` or `operator delete[]`, _S_ shall be the target scope of an allocation function named `operator new[]` and a deallocation function named `operator delete[]`.]{.add}
>
...
> [2]{.pnum} [Each candidate that is not a usual deallocation function ([basic.stc.dynamic.deallocation]) is discarded.]{.add}
>
> ::: add
> ::: note
