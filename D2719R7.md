...
::: draftnote
Is it fine to have "size parameter" italicized three times?
:::
...
:::
::: draftnote
Question to CWG: are we fine with "address parameter", or we want something with the word "storage"?
:::
...
>
> [The allocation function is selected by overload resolution using the argument list termed _L_ as specified in [over.match.new]]{.add}.
>
> [. . .]
>
...
>
> - [30.a]{.pnum} [_S_ be a set of candidates that consists of the results of a search for deallocation function's name as specified in [over.match.new], and]{.add}
...
::: draftnote
Question to CWG: Should "(de)alloaction function's name" be renamed to "(de)allocation operator's name"? Do we want "the name of the deallocation operator"?
:::
::: draftnote
Should we try to move the note after p7 somewhere? Its contents feel rather close to [basic.stс.dynamic.general]/2.
:::
::: draftnote
Do we need this "unless..." part at the end in the selection of type-aware deallocation functions? If so, for matching dellocation functions, too?
:::
...
>     - [10b.2.1.1]{.pnum} "pointer to `C`" if _F_ is a destroying operator delete, where `C` is the type of the deleted object, and
...
>
> ::: note
> Any cv-qualifiers in the type of the deleted object are ignored when forming [this]{.rm} [the address]{.add} argument.
> :::
>
> ::: note
...
:::
::: draftnote
Is the note about cv-qualifiers supported by the existing wording?
:::
...
## General [over.match.funcs.general]{.sref .unnumbered}
...
::: draftnote
How clear it is that the sets of arguments are ordered?
:::
::: draftnote
Discarding candidates that are destroying operators delete should resolve CWG2623 "Invoking destroying `operator delete` for constructor failure". Or do we instead want to make it ill-formed to select them?
:::
...
::: draftnote
Question for CWG: does p4 needs to be predicated on the presence of a type-aware candidate and on the fact that the selected allocation function is a placement one? More broadly, this is a question how independent [over.match.new] is from [expr.new], which specifies the same predicates before referring to overload resolution.
:::
...
> ::: add
> ::: note
> [The set of candidate functions for a matching deallocation function of a #new-expression# is specified in [over.match.new].]{.add}
> :::
> :::
...
::: draftnote
Vlad wishes he could structure the wording in a way that makes both this note and the note after p2 too obvious to bother including _without regressing anything else_.
:::

Move the note that follows [expr.delete] paragraph 8 after paragraph 2 and apply the following changes:

> [2]{.pnum} [Each candidate that is not a usual deallocation operator ([basic.stc.dynamic.deallocation]) is removed from the set of candidate functions.]{.add}
>
> ::: add
> ::: note
> [This applies to sets of candidate functions specified in both this subclause and in [over.match.new].]{.add}
> :::
> :::
>
> ::: note
...
> :::
>
> [3]{.pnum} [If one of the candidates is a type-aware deallocation function, overload resolution selects a deallocation function using an argument list that consists of, in order:]{.add}
>
