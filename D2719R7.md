...
>
> template <typename U>
> void* operator new(W<U>::B, std::size_t, std::align_val_t); // not a type-aware allocation operator
>
> template <typename X>
> class S {
>   using C = std::type_identity<S>;
>   void* operator new(C, std::size_t, std::align_val_t);     // a type-aware allocation operator
> };
>
> template <typename X>
> void* S<X>::operator new(std::type_identity<S<X>>, std::size_t, std::align_val_t) { return {}; };    // a type-aware allocation operator
> ```
> :::
> :::
>
...
> [1]{.pnum} An allocation [function that is not a class member function]{.rm} [operator]{.add} shall [either be a class member or]{.add} belong to the global scope and not have a name with internal linkage.
>
...
> [12]{.pnum} A #new-expression# may obtain storage for the object by calling an allocation function ([basic.stc.dynamic.allocation]).
...
> [7]{.pnum} [If a deallocation function is called, it is `operator delete` for a single-object delete expression or `operator delete[]` for an array delete expression.]{.rm}
...
> [The _deallocation size_ is the sum of the size of the deleted object and the array allocation overhead in the case of array delete expression, and the size of the deleted object otherwise.]{.add}
...
:::
::: draftnote
Do we need this "unless..." part at the end of p9 in the selection of type-aware deallocation functions? If so, for matching dellocation functions, too?
:::
...
> [13]{.pnum} The deallocation function's name is looked up by searching for it in the scope of the promise type.
...
> [1]{.pnum} Any allocation [function]{.rm} [operator]{.add} for a class `T` is a static member (even if not explicitly declared `static`).
>
> [2]{.pnum} [[. . .]]{.example}
>
...
> [2]{.pnum} The set of candidate functions can contain both member and non-member functions to be resolved against the same argument list.
...
> The argument list is the same as that of _F_, except that the value returned from the call to _F_ is inserted after the first argument.]{.add}
>
> ::: add
> ::: note
> [The parameter-type-list of the selected matching deallocation function has to satisfy additional criteria for a program to be well-formed ([expr.new]).]{.add}
> :::
> :::
>
> [5]{.pnum}
...
> [1]{.pnum} [When selecting a deallocation function for a #delete-expression# _E_ whose operand is of type "pointer to _cv_ `U`", the set of candidate functions consists of the results of a search for the deallocation function's name ([expr.delete]) in the following scopes:]{.add}
>
...
> [3]{.pnum} [Each candidate that is not a usual deallocation operator ([basic.stc.dynamic.deallocation]) is removed from the set of candidate functions.]{.add}
>
> ::: note
...
> :::
>
> [4]{.pnum} [If one of the candidates is a type-aware deallocation function, overload resolution selects a deallocation function using an argument list that consists of, in order:]{.add}
>
