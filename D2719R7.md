...
> [[basic.stc.dynamic.allocation]/1]{.pnum} An allocation [function that is not a class member function]{.rm} [operator]{.add} shall [either be a class member or]{.add} belong to the global scope and not have a name with internal linkage.
>
...
> [[basic.stc.dynamic.deallocation]/1]{.pnum} A deallocation function [that is not a class member function]{.rm} shall [either be a class member or]{.add} belong to the global scope and not have a name with internal linkage.
...
> [3-]{.pnum} [The declared return type of a deallocation operator _F_ shall be the same as ([temp.over.link]) `void`.]{.add}
> [The following restrictions apply to the parameters of _F_:]{.add}
>
> - [[3-.1]{.pnum} If _F_ is a type-aware deallocation function, then _F_ shall have at least four parameters, where]{.add}
>   - [[3-.1.1]{.pnum} the second parameter is termed the _address parameter_,]{.add}
>   - [[3-.1.2]{.pnum} the third parameter shall have a type that is the same as `std::size_t` ([support.types]), and]{.add}
>   - [[3-.1.3]{.pnum} the fourth parameter shall have a type that is the same as `std::align_val_t` ([support.dynamic]).]{.add}
> - [[3-.2]{.pnum} Otherwise, if _F_ is a function template, then _F_ shall have at least two parameters, where the first parameter is termed the _address parameter_.]{.add}
> - [[3-.3]{.pnum} Otherwise, _F_ shall be a function that has at least one parameter, where the first parameter is termed the _address parameter_.]{.add}
>
...
> - [3.2]{.pnum} a deallocation function [which is not a function template specialization and]{.add} whose parameters after the [first]{.rm} [address parameter]{.add}
...
> :::
>
> [8a]{.pnum} [For every #delete-expression# _E_ a static deallocation function is selected as specified below. If]{.add}
>
> - [8a.1]{.pnum} [the keyword `delete` in _E_ is not preceded by the unary `​::`​ operator,]{.add}
> - [8a.2]{.pnum} [the operand is a pointer to _cv_ `T`, where `T` is a class type or (possibly multidimensional) array thereof, and]{.add}
> - [8a.3]{.pnum} [`T` has a virtual destructor _D_,]{.add}
>
> [then the selected deallocation function is the deallocation function that was determined at the point of definition of _D_ ([class.dtor]).
> Otherwise, the selected deallocation function is the static deallocation function.]{.add}
>
> ::: add
> ::: note
> [The static deallocation function is selected and semantically checked even if it does not become the selected deallocation function.]{.add}
> :::
> :::
>
> [9]{.pnum} [The static deallocation function is selected by overload resolution using the set of candidate functions termed _S_ and the argument list termed _L_ as specified in [over.match.delete].]{.add}
...
Do not change [class.dtor] paragraph 15:

> [[class.dtor]/15]{.pnum} At the point of definition of a virtual destructor (including an implicit definition), the non-array deallocation function is determined as if for the expression `delete this` appearing in a non-virtual destructor of the destructor's class (see [expr.delete]).
