...
> A C++ implementation provides access to, and management of, dynamic storage via the global [_allocation functions_ `operator new` and `operator new[]`]{.rm} [allocation]{.add} and [the global _deallocation functions_ `operator delete` and `operator delete[]`]{.rm} [deallocation operators]{.add}.
>
...
::: draftnote
Should the last sentence in p1 use "functions" or "operators"?
:::
...
::: draftnote
Davis was not happy about the new note, but Vlad doesn't exactly understand his concern: [minutes](https://wiki.isocpp.org/2026-06_Brno:CoreWorkingGroup#:~:text=davis%3A%20makes%20it%20sound%20like%20other%20function%20templates%20might%20be%20possible.).
:::
...
:::
::: draftnote
"Address parameter" or "pointer parameter"?
:::
...
>
> [The allocation function is selected by overload resolution using the argument list _L_ as specified in [over.match.alloc]]{.add}.
>
> [. . .]
>
...
>
> [29a]{.pnum} [Otherwise, if the selected allocation function is type-aware, the matching deallocation function _F_ is selected by overload resolution using the argument list _M_ as specified in [over.match.alloc].
...
>
> - [30.a]{.pnum} [_S_ be a set of candidates that consists of the results of a search for deallocation function's name as specified in [over.match.alloc], and]{.add}
...
>
> [If]{.rm} [The selected deallocation function (if any) shall be]{.add} a [non-deleted]{.add} usual deallocation function [is selected, the program is ill-formed]{.rm} [accessible from the point where the #new-expression# appears]{.add}.
...
> :::
>
> [9]{.pnum} [The deallocation function to be called is selected by overload resolution using the candidate set _S_ and the argument list _L_ as specified in [over.match.delete].]{.add}
> [The]{.rm} [If overload resolution fails, the]{.add} deallocation function to be called is selected [from _S_]{.add} as follows:
>
...
::: draftnote
Should "(de)alloaction function's name" be renamed to "(de)allcation operator's name"?
:::
::: draftnote
...
>   - [10b.2.4]{.pnum} If _F_ has a parameter of type `std::align_val_t` ([support.dynamic]), then a value of `std::align_val_t` equal to the deallocation alignment.
> :::
>
> ::: note
...
:::
::: draftnote
Is the definition of deallocation alignment correct in the presence of a placement argument of type `std::align_val_t`?
:::
::: draftnote
...
> [1]{.pnum} [When selecting an allocation or deallocation function for a given #new-expression# _E_ whose allocated type is `U`, the set of candidate functions consists of the results of a search for the allocation or deallocation function's name ([expr.new], [expr.delete]), respectively, in the following scopes:]{.add}
>
...
>
> [Each candidate that is a destroying operator delete ([basic.std.dynamic.deallocation]) is removed from the set of candidates.]{.add}
>
> [2]{.pnum} [Let]{.add}
>
...
> :::
>
> [4]{.pnum} [If the selected allocation function is not a placement allocation function and there is a candidate that is a type-aware allocation function, then overload resolution selects a matching deallocation function for _E_.
> The argument list is the same as for the selected allocation function, except that the value returned from the call to the selected allocation function is inserted after the first argument.]{.add}
>
> ::: add
> ::: note
...
> [1]{.pnum} [When selecting a deallocation function for a given #delete-expression# _E_ whose operand is of type "pointer to _cv_ `U`", the set of candidate functions consists of the results of a search for the deallocation function's name ([expr.delete]) in the following scopes:]{.add}
>
...
> - [1.2]{.pnum} [Otherwise, or if no declarations were found, in the global scope.]{.add}
>
> [If any of the candidates is a destroying operator delete, each candidate that is not a destroying operator delete is removed from the set of candidates.]{.add}
>
> ::: add
> ::: note
...
::: draftnote
In [[expr.delete]/8.1](http://eel.is/c++draft/expr.delete#8.1), is it possible that the target scope of virtual destructor of the dynamic type `T` is different from the (class) scope associated with `T`? If so, we can shorten the "of" chain at the end of 1.1.1.
:::
...
> [2]{.pnum} [Each candidate that is not a usual deallocation operator ([basic.stc.dynamic.deallocation]) is removed from the set of candidates.]{.add}
>
> ::: add
> ::: note
