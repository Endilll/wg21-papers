...
+=====================+=================================+=================================+===============================+=================================+
| Name lookup         | [[over.match.new]/1             | [[over.match.new]/1             | [[expr.new]/30](#expr.new-30) | [[over.match.new]/1             |
|                     | ](#over.match.new-1)            | ](#over.match.new-1)            |  defers to                    | ](#over.match.new-1)            |
|                     |                                 |                                 | [[over.match.new/1]           |                                 |
|                     |                                 |                                 | ](#over.match.new-1)          |                                 |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+
| Candidate           | None                            | None                            | [[over.match.new]/1           | [[over.match.new]/1             |
| restrictions        |                                 |                                 | ](#over.match.new-1)          | ](#over.match.new-1)            |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+
| Argument            | [[over.match.new]/2             | [[over.match.new]/2             | [[expr.new]/30](#expr.new-30) | [[over.match.new]/4](           |
| list                | ](#over.match.new-2)            | ](#over.match.new-2)            | derives from                  | #over.match.new-4) derives from |
|                     |                                 |                                 | [[expr.new]/20                | [[expr.new]/20                  |
|                     |                                 |                                 | ](#expr.new-20)               | ](#expr.new-20)                 |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+
| Selection           | [[expr.new]/20](#expr.new-20)   | [[expr.new]/20](#expr.new-20)   | [[expr.new]/30                | [[expr.new]/29a                 |
|                     | defers to                       | defers to                       | ](#expr.new-30)               | ](#expr.new-29a) defers to      |
|                     | [[over.match.new]](             | [[over.match.new]](             |                               | [[over.match.new]](             |
|                     | #new-expression-over.match.new) | #new-expression-over.match.new) |                               | #new-expression-over.match.new) |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+
| Selected function   | Overload resolution             | Overload resolution             | [[expr.new]/30                | [[expr.new]/29a                 |
| restrictions        |                                 |                                 | ](#expr.new-30)               | ](#expr.new-29a)                |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+
| Call                | [[expr.new]/20](#expr.new-20)   | [[expr.new]/20](#expr.new-20)   | [[expr.new]/30                | [[expr.new]/29a](#expr.new-29a) |
| arguments           | derives from                    | derives from                    | ](#expr.new-30)               | derives from [[expr.new]/20     |
|                     | [[over.match.new]/2             | [[over.match.new]/2             |                               | ](#expr.new-20)                 |
|                     | ](#over.match.new-2)            | ](#over.match.new-2)            |                               |                                 |
+---------------------+---------------------------------+---------------------------------+-------------------------------+---------------------------------+

Table: Selection of allocation and deallocation functions (allocation-like)
...
|                     | [/3](#over.match.delete-3) | [/3](#over.match.delete-3)        | [[over.match.delete]/3            | [[over.match.delete]/3            |
|                     |                            |                                   | ](#over.match.delete-3)           | ](#over.match.delete-3)           |
+---------------------+----------------------------+-----------------------------------+-----------------------------------+-----------------------------------+
| Argument            | Not needed                 | [[over.match.delete]/4            | Not needed                        | [[over.match.delete]/4            |
| list                |                            | ](#over.match.delete-4)           |                                   | ](#over.match.delete-4)           |
+---------------------+----------------------------+-----------------------------------+-----------------------------------+-----------------------------------+
| Selection           | [[expr.delete]/9           | [[expr.delete]/9](#expr.delete-9) | [[expr.delete]/9                  | [[expr.delete]/9](#expr.delete-9) |
|                     | ](#expr.delete-9)          | defers to [[over.match.delete]    | ](#expr.delete-9)                 | defers to [[over.match.delete]    |
|                     |                            | ](#over.match.delete-9),          |                                   | ](#over.match.delete-9),          |
|                     |                            | falls back to [[expr.delete]/9    |                                   | falls back to [[expr.delete]/9    |
|                     |                            | ](#expr.delete-9)                 |                                   | ](#expr.delete-9)                 |
+---------------------+----------------------------+-----------------------------------+-----------------------------------+-----------------------------------+
| Selected function   | [[expr.delete]/9           | Overload resolution,              | Overload resolution,              | Overload resolution,              |
| restrictions        | ](#expr.delete-9)          | [[expr.delete]/9](#expr.delete-9) | [[expr.delete]/9](#expr.delete-9) | [[expr.delete]/9](#expr.delete-9) |
+---------------------+----------------------------+-----------------------------------+-----------------------------------+-----------------------------------+
...
Table: Selection of allocation and deallocation functions (deallocation-like)
...
> :::
>
> [2]{.pnum} An allocation function attempts to allocate the [requested]{.rm} amount of storage [specified by the size parameter]{.add}. If it is successful, it returns the address of the start of a block of storage whose length in bytes is at least as large as the [requested]{.rm} [specified]{.add} size.
...
:::
::: draftnote
As specified, p29a will not pass default arguments of the selected allocation function to the matching dealloation function. How happy we are about this? This is the second half of [CWG1628](https://cplusplus.github.io/CWG/issues/1628.html) "Deallocation function templates".
:::
...
> :::
>
> [9]{.pnum} [The deallocation function to be called is selected by overload resolution using the set of candidate functions termed _S_ and the argument list termed _L_ as specified in [over.match.delete].]{.add}
> [The]{.rm} [If overload resolution fails or was not performed, the]{.add} deallocation function to be called is selected [from _S_]{.add} as follows:
>
...
> - [9.1a]{.pnum} [Each candidate that is a type-aware deallocation function is eliminated from further consideration.]{.add}
...
> ::: add
> ::: note
> If there is a candidate that is a type-aware deallocation operator, overload resolution attempts to determine the best viable function among the candidates ([over.match.delete]). Otherwise, or if there is no best viable function, the selection process described above is used to select the deallocation function.
> :::
> :::

Move the second and the third note in paragraph 10 after the paragraph 10b, and apply the following changes:
...
> ::: add
> ::: note
> Since a matching deallocation function is called when a constructor throws, the lifetime of the deleted object does not start, but its type is known.
> :::
> :::
>
...
:::
::: draftnote
Do we need this "unless..." part at the end of p9 in the selection of type-aware deallocation functions? If so, for matching deallocation functions, too?
:::

::: draftnote
How does the "unless..." part of p9 interact with deallocation functions selected by overload resolution? Do we need a similar exception there?
:::
...
>
> [5]{.pnum} Access to the deallocation function is checked statically, even if a different one is actually executed.
>
> [[. . .]]{.example}
>
> [6]{.pnum}
...
> ::: add
> ::: note
> [If a call to the selected allocation function uses default arguments, they will influence the selection of a matching deallocation function. The parameter-type-list of the selected matching deallocation function has to satisfy additional criteria for a program to be well-formed ([expr.new]).]{.add}
> :::
> :::
>
> [5]{.pnum}
...
>
> - [1.1]{.pnum} [If _E_ does not begin with `::` and `U` is a class type:]{.add}
>   - [1.1.1]{.pnum} [if _E_ is a single-object delete expression and `U` has a virtual destructor, then in the target scope of the virtual destructor of the dynamic type of the operand,]{.add}
>   - [1.1.2]{.pnum} [otherwise, in the scope associated with `U`.]{.add}
> - [1.2]{.pnum} [Otherwise, or if no declarations were found, in the global scope.]{.add}
>
...
::: draftnote
In [[expr.delete]/8.1](http://eel.is/c++draft/expr.delete#8.1), is it possible that the target scope of virtual destructor of the dynamic type `T` is different from the (class) scope associated with `T`? If that is never the case, we can shorten the "of" chain at the end of 1.1.1.
:::

::: draftnote
Vlad wishes he could structure the wording for the candidate set for matching deallocation function (in non-placement case) in a way that renders notes too obvious to include in the wording _without regressing anything else_.
:::
...
::: draftnote
Additional places where we might need "function" → "operator" replacement: [temp.func.order], [temp.deduct.decl], [except.spec] .
:::
