...
>
> - [[1a.1]{.pnum} If _F_ is a type-aware allocation operator, then _F_ shall have at least three parameters, where the second parameter is termed the size parameter and the third parameter is termed the _alignment parameter_.]{.add}
...
> - [[1a.3]{.pnum} Otherwise _F_ shall be a function that has at least one parameter, where the first parameter is termed the size parameter.]{.add}
>
...
> ::: add
> ::: note
> An allocation operator can have additional parameters that are not specified in this document.
> :::
> :::
>
...
::: draftnote
(CWG) Is it fine to italicize "alignment parameter" but not "size parameter" and "address parameter" (the latter two are "defined" three times)?
:::
...
>
> - [2a.1]{.pnum} If _F_ is a type-aware deallocation function, then _F_ shall have at least four parameters, where
...
> - [2a.3]{.pnum} Otherwise, _F_ shall be a function that has at least one parameter, where the first parameter is termed the address parameter.
>
...
>
> ::: note
> A deallocation operator can have additional parameters that are not specified in this document.
> :::
> :::
>
...
>
> ::: add
> The allocation function is selected by overload resolution as specified in [over.match.new].
> Its complete argument list ([over.match.viable]) is termed _L_.
> The allocation function is called with arguments in _L_.
>
> ::: example
> ```cpp
> struct S {
...
>
> - [30.a]{.pnum} [_S_ be a set of candidate functions as specified in [over.match.new], and]{.add}
...
> ::: add
> ::: note
> This does not affect matching deallocation functions selected in [expr.delete], because they are usual dealloaction functions, therefore they do not accept additional arguments.
> :::
> :::
...
> [The deallocation function's name of a #delete-expression# _E_ is `operator delete` if _E_ is a single-object delete expression, and `operator delete[]` otherwise.]{.add}
>
> ::: rm
> ::: note
