...
>
> [29a]{.pnum} [Otherwise, if the selected allocation function is type-aware, the matching deallocation function _F_ is selected by overload resolution using the argument list termed _M_ as specified in [over.match.new].
...
> struct S {
>   S() noexcept;
>   void* operator new(std::size_t, int = 42);
>   void operator delete(void*, std::size_t);
> };
>
> void f() {
>   new S(); // no matching deallocation function
> }
> ```
> :::
> :::
...
>
> ::: add
> [4]{.pnum} If there is a candidate that is a type-aware deallocation function and the selected allocation function _F_ is not a placement allocation function, then overload resolution selects a matching deallocation function for _E_.
...
>
> ::: note
> If only a placement deallocation function [is]{.rm} [was]{.add} found in a class [scope]{.add}, the program is ill-formed because the lookup set is empty ([basic.lookup]).
> :::
>
> ::: add
